...
function renderVariables(vars) {
  ...
  for (const v of vars) {
    ...
    row.className = "variable-row " + typeClass;
    ...
    badgeEl.className = "variable-type-label";
    badgeEl.textContent = badgeText;

    const typeBadgeEl = document.createElement("span");
    typeBadgeEl.className = "variable-type-badge";
    typeBadgeEl.textContent = v.type;

    const valDisplay = document.createElement("input");
    valDisplay.type = "text";
    valDisplay.value = JSON.stringify(v.value);
    valDisplay.className = "variable-value" + (isReadonly ? " readonly" : "");
    valDisplay.readOnly = isReadonly;
    if (isReadonly) {
      valDisplay.title = v.name + " = " + v.type;
    }

    row.appendChild(nameEl);
    row.appendChild(badgeEl);
    row.appendChild(typeBadgeEl);
    row.appendChild(valDisplay);
    variablesList.appendChild(row);
  }
  variablesSection.style.display = "block";
  rawInputSection.style.display = "block";
}

function collectAllInputValues() {
  const inputs = {};
  if (!variableDefs || !variableDefs.length) return inputs;
  for (const v of variableDefs) {
    if (!v.isInput) continue;
    const allRows = variablesList.querySelectorAll(".variable-row");
    for (const row of allRows) {
      const nameEl = row.querySelector(".variable-name");
      if (!nameEl || nameEl.textContent !== v.name) continue;
      const valEl = row.querySelector("input.variable-value:not(.readonly)");
      if (!valEl) break;
      let rawVal = valEl.value;
      try {
        inputs[v.name] = JSON.parse(rawVal);
      } catch {
        inputs[v.name] = rawVal;
      }
      break;
    }
  }
...
function updateVariableValuesFromTick(responseVars) {
  if (!responseVars || !variableDefs.length) return;
  const allRows = variablesList.querySelectorAll(".variable-row");
  for (const row of allRows) {
    const nameEl = row.querySelector(".variable-name");
    if (!nameEl) continue;
    const varName = nameEl.textContent;
    const valEl = row.querySelector("input.variable-value");
    if (!valEl) continue;
    if (responseVars[varName] !== undefined) {
      valEl.value = JSON.stringify(responseVars[varName]);
    }
  }
}
    ...
    if (isSCTX) {
      try {
        ...
        if (!isConfigured) {
          ...
          showStatus(
            connectionStatus,
            "Model compiled and loaded from file",
            "success",
          );
        }
      } catch (err) {
        ...
        showStatus(
          connectionStatus,
          "Failed to read file: " + toErrorString(err),
          "error",
        );
      }
    } else {
        ...
        if (!ok) {
          ...
          showStatus(
            ...
            "JSON model loaded — model: " +
              (modelData ? Object.keys(modelData).join(", ") : "(unknown)"),
            "success",
          );
        }
      } catch (err) {
...
async function doSetup(data) {
  ...
  if (!resp.ok) {
    ...
    showStatus(
      connectionStatus,
      "Setup failed: " + toErrorString(errBody),
      "error",
    );
    modelData = null;
    return false;
...
async function compileAndLoadModel(text) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      showStatus(
        textCompileStatus,
        "Compilation failed: " + toErrorString(errBody),
        "error",
      );
      return false;
    }
    ...
    showStatus(
      textCompileStatus,
      "Compilation failed: " + toErrorString(err),
      "error",
    );
    modelData = null;
  }
...
resetBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    showStatus(
      connectionStatus,
      "Reset failed: " + toErrorString(err),
      "error",
    );
    advanceTickBtn.disabled = false;
    advanceTickBtn.textContent = "Advance Tick";
...
function getInputSource() {
  if (rawInputSection && rawInputSection.hasAttribute("open")) {
    try {
      const parsed = JSON.parse(tickInput.value || "{}");
      log("Using raw input editor");
      return Array.isArray(parsed) ? parsed : [parsed];
    } catch (err) {
      return null;
    }
  }
  return collectAllInputValues();
}
...
async function executeTicks() {
  ...
  if (rawInputSection && rawInputSection.hasAttribute("open")) {
    ...
    baseInputs = collectAllInputValues();
  }
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    tickOutput.textContent +=
      ...
      line +
      terminated;
    updateVariableValuesFromTick(result.variables);
...
advanceTickBtn.addEventListener("click", async () => {
  ...
  if (rawInputSection && rawInputSection.hasAttribute("open")) {
    ...
    baseInputs = Array.isArray(inputSource) ? inputSource[0] : inputSource;
  } else {
    baseInputs = collectAllInputValues();
  }
